...
import { Zap, Clock, Plus, List, Calendar as CalendarIcon, Search, Edit, Eye, ChevronLeft, ChevronRight, Check, User, CheckCircle, MessageSquare, Download, Upload, ChevronDown, Ban, Filter, X } from 'lucide-react'
...
export default function ShiftListView() {
  ...
  const monthlyShiftsData: Record<number, MonthData> = {
  ...
  const getMonthData = (month: number): MonthData => monthlyShiftsData[month] || { total: 0, morning: 0, afternoon: 0, night: 0 }

  const colorMap: ColorMap = {
    status: { green: 'bg-green-100 text-green-800 hover:bg-green-100', yellow: 'bg-yellow-100 text-yellow-800 hover:bg-yellow-100', red: 'bg-red-100 text-red-800 hover:bg-red-100', blue: 'bg-blue-100 text-blue-800 hover:bg-blue-100', gray: 'bg-gray-100 text-gray-800 hover:bg-gray-100' },
    shift: { blue: 'bg-blue-100 text-blue-800 hover:bg-blue-100', orange: 'bg-orange-100 text-orange-800 hover:bg-orange-100', purple: 'bg-purple-100 text-purple-800 hover:bg-purple-100' }
  }

  const getStatusColor = (color: string) => colorMap.status[color] || colorMap.status.green
  const getShiftColor = (color: string) => colorMap.shift[color] || colorMap.shift.blue
  ...
  const FilterSection = ({ title, items, checked, onToggle }: FilterSectionProps) => (
    <div>
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex space-x-8 -mb-px">
            ...
            <div className="flex flex-wrap gap-2 justify-end">
              <Button onClick={() => console.log('Importing')} variant="outline"><Upload className="w-4 h-4 mr-2" />Import</Button>
              <Button onClick={() => console.log('Exporting')} variant="outline"><Download className="w-4 h-4 mr-2" />Export</Button>
              <Button onClick={() => setIsAutoScheduleModalOpen(true)} variant="outline" className="border-purple-200 text-purple-700 hover:bg-purple-50"><Zap className="w-4 h-4 mr-2" />Auto Schedule</Button>
              <Button onClick={() => setIsHistoryModalOpen(true)} variant="outline" className="border-amber-300 text-amber-600 hover:bg-amber-50"><Clock className="w-4 h-4 mr-2 text-amber-500" />History</Button>
              <Button onClick={() => setIsAssignModalOpen(true)} variant="outline" className="border-blue-200 text-white hover:bg-blue-50 bg-blue-400"><Plus className="w-4 h-4 mr-2" />Assign Shift</Button>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {activeTab === 'list' && (
          <div className="bg-white shadow-sm rounded-xl border border-gray-200">
            <div className="p-5 border-b border-gray-200">
              <div className="flex items-center justify-end gap-3">
                <div className="relative w-64">
                  ...
                  <Input type="text" placeholder="Search employee..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
                </div>
                  ...
                  <PopoverContent className="w-[280px] p-0" align="end">
                    <div className="p-3 border-b border-gray-200 flex items-center justify-between bg-gray-50">
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="ghost" size="sm" onClick={clearTempFilters} className="text-gray-600 hover:text-gray-900">Clear all</Button>
                    ...
                    <div className="p-3 border-t border-gray-200 flex items-center justify-between bg-gray-50">
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" onClick={() => setFiltersOpen(false)} className="min-w-[80px]">Cancel</Button>
                        <Button size="sm" onClick={applyFilters} className="bg-blue-600 hover:bg-blue-700 text-white min-w-[80px]"><Check className="w-4 h-4 mr-2" />OK</Button>
                      </div>
                    </div>
                  </PopoverContent>
                </Popover>
            ...
            <div className="relative">
              <div className="overflow-auto max-h-[calc(100vh-350px)] hide-scrollbar">
                <Table>
                  <TableHeader className="sticky top-0 bg-white z-20 border-b border-gray-200">
                    <TableRow>
                      {['Employee', 'Department', 'Shift Type', 'Shift Pattern', 'Start Date', 'End Date', 'Status', 'Actions'].map(header => (
                        <TableHead key={header} className="bg-white">{header}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredShifts.length > 0 ? filteredShifts.map(shift => (
                      <TableRow key={shift.id}>
                        ...
                        <TableCell><span className="text-sm text-gray-900">{shift.department}</span></TableCell>
                        <TableCell><Badge className={getShiftColor(shift.shiftColor)}>{shift.shiftType}</Badge></TableCell>
                        <TableCell>
                          <div className="text-sm text-gray-900">{shift.shiftPattern}</div>
                          {shift.recurringPattern && <div className="text-xs text-gray-500 mt-1">{shift.recurringPattern}</div>}
                        </TableCell>
                        <TableCell><div className="text-sm text-gray-900">{shift.startDateDisplay}</div></TableCell>
                        <TableCell><div className="text-sm text-gray-900">{shift.endDateDisplay}</div></TableCell>
                        <TableCell><Badge className={getStatusColor(shift.statusColor)}>{shift.status}</Badge></TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => { setSelectedShift(shift); setIsEditModalOpen(true) }} title="Edit"><Edit className="w-4 h-4" /></Button>
                            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => { setSelectedShift(shift); setIsViewModalOpen(true) }} title="View"><Eye className="w-4 h-4" /></Button>
                            {shift.status !== 'Suspended' && <Button size="icon" variant="ghost" className="h-8 w-8 text-orange-600 hover:text-orange-700 hover:bg-orange-50" onClick={() => { setSelectedShift(shift); setDeactivateDialogOpen(true) }} title="Deactivate"><Ban className="w-4 h-4" /></Button>}
                          </div>
                        </TableCell>
                      </TableRow>
                    )) : (
                      <TableRow>
                        <TableCell colSpan={8} className="text-center py-10">
                          <Search className="w-12 h-12 mb-3 text-gray-300 mx-auto" />
            ...
            <div className="px-6 py-4 flex items-center justify-between border-t border-gray-200">
              <div className="text-sm text-gray-700">
                Showing <span className="font-medium">{filteredShifts.length > 0 ? 1 : 0}</span> to <span className="font-medium">{filteredShifts.length}</span> of <span className="font-medium">{filteredShifts.length}</span> results
              </div>
              <div className="flex items-center gap-2">
        ...
        {/* SIMPLE CALENDAR WITH BLUE/VIOLET & RED INTENSITY DOTS */}
        {activeTab === 'calendar' && (
          <div className="bg-white shadow-sm rounded-xl border border-gray-200">
            {/* Calendar Header */}
            ...
            <div className="p-6 bg-gray-50">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {months.map((month, index) => {
                  ...
                  // Calculate intensity (red dots) based on total shifts
                  const getIntensity = () => {
                  ...
                  return (
                    <div
                      key={month}
                      className={`relative bg-white border rounded-lg p-5 hover:shadow-md transition-all cursor-pointer ${isBlue ? 'border-blue-200' : 'border-violet-200'
                        }`}
                    >
      ...
      <style>{`
        .scrollbar-hide, .hide-scrollbar { scrollbar-width: none; -ms-overflow-style: none; }
        .scrollbar-hide::-webkit-scrollbar, .hide-scrollbar::-webkit-scrollbar { display: none; width: 0; height: 0; }
      `}</style>
    </div>
  )
}
...
// Edit Shift Modal
function EditShiftModal({ shift, onClose }: EditShiftModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); console.log('Updated:', formData); onClose() }
...
// View Shift Modal
function ViewShiftModal({ shift, onClose }: ViewShiftModalProps) {
  ...
  const InfoSection = ({ title, icon: Icon, bgColor, items }: InfoSectionProps) => (
    <div className={`${bgColor} rounded-lg p-5`}>
      ...
      <div className="space-y-3">{items.map((item, i: number) => (
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[95vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto py-6 space-y-6 px-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          <style>{`div::-webkit-scrollbar { display: none; }`}</style>
          ...
          <InfoSection title="Shift Information" icon={Clock} bgColor="bg-blue-50" items={[
            { label: 'Shift Type', value: shift.shiftType, badge: true, color: colorMap[shift.shiftColor] },
          ...
          <InfoSection title="Status Information" icon={CheckCircle} bgColor="bg-green-50" items={[
            { label: 'Current Status', value: shift.status, badge: true, color: colorMap[shift.statusColor] },
